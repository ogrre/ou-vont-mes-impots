...
class StateBudgetRevenueCsvImporter implements DatasetImporter
{
    ...
    {
        if (($datasetFile->metadata['importer'] ?? null) !== 'state_budget_revenue_csv' || ! is_file($path)) throw new InvalidSourceDataException('Descripteur ou fichier de recettes CSV invalide.');
        $checksum = hash_file('sha256', $path);
        try { $batch = ImportBatch::query()->create(['dataset_id' => $datasetFile->dataset_id, 'dataset_file_id' => $datasetFile->id, 'filename' => basename($path), 'checksum' => $checksum, 'status' => ImportStatus::Running, 'started_at' => now(), 'metadata' => ['descriptor' => $datasetFile->slug]]); } catch (UniqueConstraintViolationException) { throw new DuplicateImportException("Ce contenu a déjà été importé pour {$datasetFile->slug}."); }
        $read = 0; $written = 0;
        try {
            DB::transaction(function () use ($datasetFile, $path, $batch, &$read, &$written): void {
                ...
                $handle = fopen($path, 'rb'); if ($handle === false) throw new InvalidSourceDataException('CSV de recettes illisible.');
                try {
                    $headers = fgetcsv($handle); if ($headers === false || count($headers) < 10) throw new InvalidSourceDataException('En-têtes de recettes CSV incomplets.');
                    while (($values = fgetcsv($handle)) !== false) {
                        if (count($values) < 10) continue;
                        $read++; $level = (int) trim((string) $values[0]); $category = $this->clean($values[1]); $section = $this->clean($values[2]); $line = $this->clean($values[4]);
                        $label = $line !== '' ? $line : ($section !== '' ? $section : $category); if ($label === '') continue;
                        ...
                        $parent = null;
                        if ($level > 0) $parent = ClassificationItem::query()->where('classification_id', $classification->id)->where('metadata->csv_path', implode('|', array_slice($pathParts, 0, $level)))->first();
                        ...
                        foreach ([['year' => 2024, 'column' => 7, 'stage' => BudgetStage::InitialBudget, 'status' => ObservationStatus::InitialEstimate, 'field' => 'Total des prévisions'], ['year' => 2024, 'column' => 9, 'stage' => BudgetStage::Execution, 'status' => ObservationStatus::Executed, 'field' => 'Total des recettes**']] as $semantics) {
                            $raw = $this->clean($values[$semantics['column']] ?? ''); if ($raw === '') continue;
        ...
        } catch (Throwable $exception) { $batch->update(['status' => ImportStatus::Failed, 'completed_at' => now(), 'rows_read' => $read, 'rows_rejected' => 1, 'error_message' => $exception->getMessage()]); throw $exception; }
        return $batch->fresh();
    }
    private function clean(mixed $value): string { return trim(preg_replace('/\s+/u', ' ', str_replace(["\xC2\xA0", "\xEF\xBB\xBF"], ' ', (string) $value)) ?? (string) $value); }
    private function euro(string $value): string { return number_format((float) str_replace([',', ' '], ['.', ''], $value), 2, '.', ''); }
}
